...
const ModalRutasEmpresa = ({ open, onClose, initialData = null, onSubmit, onDone }) => {
    ...
    if (open) {
      ...
      if (initialData) {
        setForm({
          ...
          // empresa will be set from the logged user's empresa_id; keep initialData as fallback
          empresa: user?.id_empresa ? String(user.id_empresa) : (initialData?.id_empresa ? String(initialData.id_empresa) : (initialData.empresa_id ? String(initialData.empresa_id) : '')),
        ...
        setForm({ ...defaultForm, empresa: user?.id_empresa ? String(user.id_empresa) : '' });
      }
    }
  }, [open, initialData]);
  ...
  useEffect(() => {
    // Ensure form.empresa reflects the logged user's empresa_id when available.
    if (!open) return;
    if (user?.id_empresa && form.empresa !== String(user.id_empresa)) {
      setForm((f) => ({ ...f, empresa: String(user.id_empresa) }));
      return;
    }

    // Fallback: if editing and empresas list loaded, prefer empresa from initialData when user not present
    if (initialData && !user?.id_empresa) {
  ...
  const validate = () => {
    ...
    // empresa is taken from the logged user; ensure it exists
    if (!user?.id_empresa && !form.empresa) e.empresa = 'No se encontró empresa asociada al usuario';
  ...
  const handleSubmit = async (ev) => {
    ...
    const payload = {
      ...
      // use logged user's empresa id when available
      id_empresa: user?.id_empresa ? Number(user.id_empresa) : (form.empresa ? Number(form.empresa) : null),
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                {/* Empresa is assigned from the logged-in user's company. Do not show a selectable list. */}
...
export default ModalRutasEmpresa;
